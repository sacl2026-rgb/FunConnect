...
export interface MadgwickOutput {
  ...
  freefall_samples: number;                       // max consecutive samples below threshold
  stationary: boolean;                            // device was still before the event
...
export function madgwick(samples: number[][]): MadgwickOutput {
  ...
  return {
    ...
    freefall_samples: maxFreefallRun,
    stationary: stationaryBefore,
    classification,
  };
}
